# UDP：用户数据报协议
> U D P是不可靠的面向数据报的运输层协议：进程的每个输出操作都正好产生一个UDP数据报，并组装成一份待发送的IP数据报 与面向流字符的协议不同 如 T C P，应用程序产生的全体数据与真正发送的单个IP数据报可能没有什么联系UDP数据报封装成一份I P数据报的格式.它把应用程序传给 I P层的数据发送出去，但是并不保证它们能到达目的地
...
* ICMP不可达差错（需要分片）
  ...
  *  I C M P差错试图判断从路由器 netb到主机sun之间的拨号SLIP链路的M T U从s u n到n e t b的链路的MTU：当SLIP被安装到主机s u n时是SLIP配置过程中的一部分已经通过netstat命令观察过。从另一个方向来判断它的M T U（如何用S N M P来判断)在点到点的链路中 不要求两个方向的M T U为相同值。
    * 主机 s o l a r i s上运行p i n g程序到主机b s d i，增加数据分组长度，直到看见进入的分组被分片为止。
